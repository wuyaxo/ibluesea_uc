...
public class CustomSqlSessionTemplate extends SqlSessionTemplate{
	 ...
	 * 日志类，使用Log4j进行日志输出
	 */
	private Logger logger = Logger.getLogger(CustomSqlSessionTemplate.class);
	...
	public SqlSessionFactory getSqlSessionFactory() {
		...
		if (mappers != null && mappers.length > 0) {
			className = (String) mappers[0];
			className = className.substring(0, className.lastIndexOf("."));
			Class<?> clazz = null;
			try {
				clazz = Class.forName(className);
			} catch (ClassNotFoundException e) {
				logger.error("没有该类：" + className);
			}
			if (clazz.isInterface() && clazz.isAnnotationPresent(DataSource.class)) {
				DataSource dataSource = clazz.getAnnotation(DataSource.class);
				dataSourceName = dataSource.name();
			}
		}
		SqlSessionFactory targetSqlSessionFactory = targetSqlSessionFactorys.get(dataSourceName);
